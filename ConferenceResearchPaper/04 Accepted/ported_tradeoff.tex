...
\confheader{%
 2026 2\textsuperscript{nd} International Conference on Distributed Systems, Computer Networks and Cybersecurity (ICDSCNC)
 }
...
\newcommand{\placetextbox}[3]{
 ...
 \placetextbox{.23}{0.055}{\small{979-8-3195-2964-0/26/\$31.00~\copyright 2026 IEEE}}
...
Since $(\Delta\theta)^2 > 0$, $\operatorname{sign}(\Delta o_i \Delta o_j) = \operatorname{sign}\bigl(\frac{\partial o_i}{\partial \theta_k} \frac{\partial o_j}{\partial \theta_k}\bigr)$, so a negative product (Eq.~\eqref{eq:product}) indicates a tradeoff.
...
\noindent\textbf{Rule 1 (Explicitness):} Every tradeoff, guarantee, and behavioral property must be declared and visible. No critical behavior may depend on undocumented defaults. Consistency and durability guarantees are per data product, not per system.
...
\noindent\textbf{Rule 2 (Boundaries):} Service boundaries must define explicit fault domains, ownership, and degradation paths. The control plane and data plane must be independently scalable, deployable, and recoverable. Every boundary carries a declared failure mode and recovery procedure.
...
\noindent\textbf{Rule 4 (Observability):} Every policy must declare its expected signal latency and reversal latency. Policy effects must be attributable to specific configuration changes. Metrics must be displayed alongside active policy context. Configuration changes without policy declarations are invalid.
...
\noindent\textbf{Rule 5 (Governance):} Every policy plane has a single owner. Policy changes require documented intent, known tradeoffs, expected signals, and rollback triggers. Incidents produce policy improvements. Dangerous combinations are detected before application.
...
\noindent\textbf{Rule 6 (Robustness):} A distributed system must preserve declared semantics under stress or fail explicitly with identifiable error codes. Operational procedures must not require tribal knowledge. Failure modes must be diagnosable. Behavior must remain predictable across workload phases.
...
\node[rule,right=0.15cm of r1] (r2) {Rule 2: Boundaries};
\node[rule,right=0.15cm of r2] (r3) {Rule 3: Reversibility};

\node[rule,below=0.45cm of r1] (r4) {Rule 4: Observability};
\node[rule,right=0.15cm of r4] (r5) {Rule 5: Governance};
\node[rule,right=0.15cm of r5] (r6) {Rule 6: Robustness};

\node[label]
at ($(r1.north)!0.5!(r3.north)+(0,0.42cm)$)
{Design Rules};
...
\node[plane,fill=blue!8,below=0.75cm of r4] (p1) {Consistency};
\node[plane,fill=green!8,right=0.12cm of p1] (p2) {Performance};
\node[plane,fill=orange!8,right=0.12cm of p2] (p3) {Caching};
\node[plane,fill=red!8,right=0.12cm of p3] (p4) {Durability};
\node[plane,fill=purple!8,right=0.12cm of p4] (p5) {Flow Control};

\node[label]
at ($(p1.south)!0.5!(p5.south)+(0,-0.28cm)$)
{Policy Planes};
...
(sys)
at ($(p1.south)!0.5!(p5.south)+(0,-0.95cm)$)
{Distributed System};

%---------------- Policy Connections ----------------%

\draw[dashedarrow] (p1.south) -- ($(sys.north west)!0.12!(sys.north east)$);
\draw[dashedarrow] (p2.south) -- ($(sys.north west)!0.31!(sys.north east)$);
\draw[dashedarrow] (p3.south) -- (sys.north);
\draw[dashedarrow] (p4.south) -- ($(sys.north west)!0.69!(sys.north east)$);
\draw[dashedarrow] (p5.south) -- ($(sys.north west)!0.88!(sys.north east)$);

\end{tikzpicture}
}

\caption{The six rules and five policy planes. Rules 1--6 form a dependency chain: explicitness enables boundaries, which enable reversibility, which enable observability, which enable governance, which enable robustness. The five policy planes operationalize the rules within the distributed system.}
\label{fig:rules}
\end{figure}
...
\label{sec:ahp}
For a financial transaction workload, stakeholder pairwise comparisons (via AHP procedure~\cite{saaty2008decision} described above) yield:
...
E. A. Brewer, ``Towards robust distributed systems,'' in \textit{Proceedings of the Nineteenth Annual ACM Symposium on Principles of Distributed Computing (PODC)}, Portland, OR, USA, Jul. 2000, p.~7.
...
\bibitem{pacelc}
D. J. Abadi, ``Consistency tradeoffs in modern distributed database system design,'' \textit{Computer}, vol.~45, no.~2, pp.~37--42, Feb. 2012.
\href{https://doi.org/10.1109/MC.2012.33}{doi:10.1109/MC.2012.33}

\bibitem{dynamo}
G. DeCandia, D. Hastorun, M. Jampani, G. Kakulapati, A. Lakshman, A. Pilchin, S. Sivasubramanian, P. Vosshall, and W. Vogels, ``Dynamo: Amazon's highly available key-value store,'' in \textit{Proceedings of the Twenty-First ACM Symposium on Operating Systems Principles (SOSP)}, Stevenson, WA, USA, Oct. 2007, pp.~205--220.
\href{https://doi.org/10.1145/1294261.1294281}{doi:10.1145/1294261.1294281}

\bibitem{bigtable}
F. Chang, J. Dean, S. Ghemawat, W. C. Hsieh, D. A. Wallach, M. Burrows, T. Chandra, A. Fikes, and R. E. Gruber, ``Bigtable: A distributed storage system for structured data,'' in \textit{Proceedings of the 7th USENIX Symposium on Operating Systems Design and Implementation (OSDI)}, Seattle, WA, USA, Nov. 2006, pp.~205--218. Berkeley, CA, USA: USENIX Association, 2006. [Online]. Available:
\url{https://www.usenix.org/conference/osdi-06/bigtable-distributed-storage-system-structured-data}

\bibitem{spanner}
J. C. Corbett, J. Dean, M. Epstein, A. Fikes, C. Frost, J. J. Furman, S. Ghemawat, A. Gubarev, C. Heiser, P. Hochschild, W. C. Hsieh, S. Kanthak, E. Kogan, H. Li, A. Lloyd, S. Melnik, D. Mwaura, D. Nagle, S. Quinlan, R. Rao, L. Rolig, Y. Saito, M. Szymaniak, C. Taylor, R. Wang, and D. Woodford, ``Spanner: Google's globally distributed database,'' in \textit{Proceedings of the 10th USENIX Symposium on Operating Systems Design and Implementation (OSDI)}, Hollywood, CA, USA, Oct. 2012, pp.~251--264. Berkeley, CA, USA: USENIX Association, 2012. [Online]. Available:
\url{https://www.usenix.org/conference/osdi12/technical-sessions/presentation/corbett}

\bibitem{calvin}
A. Thomson, T. Diamond, S.-C. Weng, K. Ren, P. Shao, and D. J. Abadi, ``Calvin: Fast distributed transactions for partitioned database systems,'' in \textit{Proceedings of the ACM SIGMOD International Conference on Management of Data}, Scottsdale, AZ, USA, May 2012, pp.~1--12.
\href{https://doi.org/10.1145/2213836.2213838}{doi:10.1145/2213836.2213838}

\bibitem{crdt}
M. Shapiro, N. Preguiça, C. Baquero, and M. Zawirski, ``Conflict-free replicated data types,'' in \textit{Proceedings of the 13th International Conference on Stabilization, Safety, and Security of Distributed Systems (SSS)}, Grenoble, France, Oct. 2011, \textit{Lecture Notes in Computer Science}, vol.~6976, pp.~386--400.
\href{https://doi.org/10.1007/978-3-642-24550-3_29}{doi:10.1007/978-3-642-24550-3\_29}

\bibitem{jepsen}
K. Kingsbury, \textit{Jepsen: Distributed Systems Safety Analysis}. [Online]. Available:
\url{https://jepsen.io}.
...
K. J. \AA str\"om and R. M. Murray, \textit{Feedback Systems: An Introduction for Scientists and Engineers}. Princeton, NJ, USA: Princeton University Press, 2008.
ISBN: 9780691135766.
...
\bibitem{simon}
H. A. Simon, \textit{Models of Bounded Rationality}, vols.~1--2. Cambridge, MA, USA: MIT Press, 1982.
ISBN: 9780262690861.

\bibitem{dijkstra1972humble}
E. W. Dijkstra, ``The humble programmer,'' \textit{Communications of the ACM}, vol.~15, no.~10, pp.~859--866, Oct. 1972.
\href{https://doi.org/10.1145/355604.361591}{doi:10.1145/355604.361591}

\bibitem{parnas1972criteria}
D. L. Parnas, ``On the criteria to be used in decomposing systems into modules,'' \textit{Communications of the ACM}, vol.~15, no.~12, pp.~1053--1058, Dec. 1972.
\href{https://doi.org/10.1145/361598.361623}{doi:10.1145/361598.361623}

\bibitem{lamport2001paxos}
L. Lamport, ``Paxos made simple,'' \textit{ACM SIGACT News}, vol.~32, no.~4, pp.~18--25, Dec. 2001.
\href{https://doi.org/10.1145/568425.568433}{doi:10.1145/568425.568433}

\bibitem{raft}
D. Ongaro and J. Ousterhout, ``In search of an understandable consensus algorithm,'' in \textit{Proceedings of the 2014 USENIX Annual Technical Conference (USENIX ATC)}, Philadelphia, PA, USA, Jun. 2014, pp.~305--319. Berkeley, CA, USA: USENIX Association, 2014. [Online]. Available:
\url{https://www.usenix.org/conference/atc14/technical-sessions/presentation/ongaro}

\bibitem{kafka}
J. Kreps, N. Narkhede, and J. Rao, ``Kafka: A distributed messaging system for log processing,'' in \textit{Proceedings of the ACM SIGMOD International Workshop on Networking Meets Databases (NetDB)}, Athens, Greece, Jun. 2011. [Online]. Available:
\url{https://notes.stephenholiday.com/Kafka.pdf}

\bibitem{corbato1972}
F. J. Corbat\'o, ``On building systems that will fail,'' \textit{Communications of the ACM}, vol.~34, no.~9, pp.~72--81, Sep. 1991.
\href{https://doi.org/10.1145/114669.114686}{doi:10.1145/114669.114686}

\bibitem{brewer2012cap}
E. A. Brewer, ``CAP twelve years later: How the ``rules'' have changed,'' \textit{Computer}, vol.~45, no.~2, pp.~23--29, Feb. 2012.
\href{https://doi.org/10.1109/MC.2012.37}{doi:10.1109/MC.2012.37}

\bibitem{chaos}
C. Rosenthal and N. Jones, \textit{Chaos Engineering}. Sebastopol, CA, USA: O'Reilly Media, Inc., 2020.
ISBN: 9781492043867.

\bibitem{github2018}
GitHub Engineering, ``Incident report: Database failover on October 21, Oct. 2018,'' Oct.~30, 2018. [Online]. Available:
\url{https://github.blog/news-insights/company-news/oct21-post-incident-analysis/}

\bibitem{cloudflare2020}
Cloudflare, ``Cloudflare outage on July 2, 2019,'' Jun.~24, 2020. [Online]. Available:
\url{https://blog.cloudflare.com/details-of-the-cloudflare-outage-on-july-2-2019/}

\bibitem{gitlab2017}
GitLab, ``GitLab.com database incident,'' Feb.~1, 2017. [Online]. Available:
\url{https://about.gitlab.com/2017/02/01/gitlab-dot-com-database-incident/}

\bibitem{aws2017}
Amazon Web Services, ``Summary of the Amazon S3 service disruption in the US-EAST-1 region,'' Mar.~2, 2017. [Online]. Available:
\url{https://aws.amazon.com/message/41926/}

\bibitem{saaty2008decision}
T. L. Saaty, ``Decision making with the analytic hierarchy process,'' \textit{International Journal of Services Sciences}, vol.~1, no.~1, pp.~83--98, Jan. 2008. [Online]. Available:
\url{https://www.inderscience.com/info/inarticle.php?artid=17590}

\bibitem{policyarch}
M. Sloman, ``Policy driven management for distributed systems,'' \textit{Journal of Network and Systems Management}, vol.~2, no.~4, pp.~333--360, Dec. 1994.
\href{https://doi.org/10.1007/BF02283186}{doi:10.1007/BF02283186}

\bibitem{pourmajidi2025governance}
W. Pourmajidi, L. Zhang, J. Steinbacher, T. Erwin, and A. Miranskyy, ``A Reference Architecture for Governance of Cloud Native Applications,'' \textit{IEEE Transactions on Cloud Computing}, vol.~13, no.~3, pp.~935--952, Jul.--Sep. 2025.
\href{https://doi.org/10.1109/TCC.2025.3578557}{doi:10.1109/TCC.2025.3578557}

\bibitem{cncf2023policy}
CNCF SIG Security and Kubernetes Policy Working Group, \textit{Kubernetes Policy Management White Paper}. Cloud Native Computing Foundation (CNCF), Jan. 2022. [Online]. Available:
\url{https://www.cncf.io/blog/2022/01/28/announcing-the-kubernetes-policy-management-paper/}

\bibitem{cncf2023governance}
CNCF, ``Kubernetes Policy-Based Governance, Risk and Compliance Paper,'' Oct.~19, 2023. [Online]. Available:
\url{https://www.cncf.io/blog/2023/10/19/announcing-the-kubernetes-policy-based-governance-risk-and-compliance-paper/}

\bibitem{observability2026}
A. Mahida, ``Enhancing Observability in Distributed Systems---A Comprehensive Review,'' \textit{Journal of Mathematical \& Computer Applications}, vol.~2, no.~3, pp.~1--4, Sep. 2023.
\href{https://doi.org/10.47363/JMCA/2023(2)135}{doi:10.47363/JMCA/2023(2)135}

\bibitem{theodoropoulos2023cloud}
T. Theodoropoulos, A. Makris, and K. Tserpes, ``Security in Cloud-Native Services: A Survey,'' \textit{Journal of Cybersecurity and Privacy}, vol.~3, no.~4, pp.~725--749, 2023.
\href{https://doi.org/10.3390/jcp3040036}{doi:10.3390/jcp3040036}

\bibitem{gherghe2025framework}
A. L. Gherghe, R. M. Banciu, and D. D. P{\u{a}}tra{\c{s}}cu, ``A Novel Framework for Evaluating Application Performance in Distributed Systems,'' \textit{Applied Sciences}, vol.~15, no.~23, Art.~no.~12837, 2025.
