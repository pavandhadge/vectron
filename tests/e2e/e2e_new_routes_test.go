...
func TestE2E_NewRoutes(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		nodeID := i + 1
		pdDataDirNode, err := RepoTempDir(fmt.Sprintf("pd_e2e_full_test_node%d-", nodeID))
		require.NoError(t, err)
	...
	workerDataDir, err := RepoTempDir("worker_e2e_full_test-")
	require.NoError(t, err)
	...
	// Start Reranker (required by API Gateway)
	...
	gatewayDataDir, err := RepoTempDir("gateway_e2e_new_routes-")
	require.NoError(t, err)
	...
	gatewayHTTPPort := GetFreePort()
	gatewayHTTPAddr := fmt.Sprintf("127.0.0.1:%d", gatewayHTTPPort)
	...
	gatewayEnv := []string{ // Environment variables for apigateway
		...
		fmt.Sprintf("HTTP_ADDR=%s", gatewayHTTPAddr),
