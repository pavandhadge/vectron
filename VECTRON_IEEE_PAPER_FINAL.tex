...
\title{Vectron: Democratizing Vector Search—Distributed Raft-HNSW Architecture for Edge and Commodity Infrastructure}
...
The proliferation of large language models and embedding-based AI applications has changed how organizations store and retrieve unstructured data. Contemporary AI systems represent diverse content types including text, images, audio, and multimodal data as high-dimensional vectors, commonly referred to as embeddings, enabling semantic similarity search beyond keyword-based retrieval. These applications span conversational AI with retrieval-augmented generation \cite{rag}, recommendation engines, image search, anomaly detection, and document classification. The dimensionality of these embeddings typically ranges from 128 to 1536 dimensions depending on model architecture, with many recent models producing 768-dimensional or 1536-dimensional vectors.

Deploying vector search at production scale presents engineering challenges that must be addressed simultaneously: low query latency for interactive use, storage for billions of vectors with metadata, high availability with failover, consistency across replicas, and operational simplicity. These requirements create trade-offs that existing solutions often resolve through compromises between performance, consistency, and operational complexity.

Standalone vector databases such as FAISS \cite{faiss} and Annoy \cite{annoy} offer strong single-node performance but lack distributed consensus, replication, and automatic failover needed in production. These systems are suitable for research and prototyping, but production deployment typically requires additional engineering around reliability. Distributed systems like Milvus \cite{milvus} and Weaviate \cite{weaviate} provide horizontal scaling but may rely on eventually consistent designs that can return stale results during replica lag or partitions. Cloud-native offerings can also introduce vendor lock-in and may not meet transparency or auditability requirements in regulated environments.

\subsection{Limitations of Existing Approaches}

Current vector database architectures exhibit limitations in meeting production requirements across deployment scenarios. Many systems prioritize availability over consistency, adopting AP-mode designs from the CAP theorem \cite{cap}. This can return stale results during replica lag or network partitions, which is problematic for freshness-sensitive applications such as recommendations, inventory management, or transaction monitoring.

The Hierarchical Navigable Small World algorithm, which has emerged as a widely used indexing approach for approximate nearest neighbor search in high-dimensional spaces, can require substantial memory overhead for maintaining graph connectivity structures \cite{hnsw}. This overhead can limit dataset sizes that can be accommodated on commodity hardware and increase operational costs for organizations seeking to store billions of vectors. Without compression or quantization techniques, a billion vectors of 768 dimensions each requires approximately 3 terabytes of memory for storage alone, plus additional overhead for the HNSW graph structure.
...
Vectron addresses these limitations through a principled systems architecture that makes explicit, well-reasoned trade-offs guided by production requirements rather than theoretical ideals. The system employs Raft consensus for all metadata operations including collection creation, shard assignment, and replica placement, ensuring that all nodes agree on system topology even during network partitions. This reduces the risk of configuration drift and split-brain scenarios in eventually consistent settings, where different nodes may otherwise have divergent views of cluster state.

The system implements HNSW with multiple performance optimizations including AVX2 SIMD instructions for distance computation, int8 quantization reducing per-vector storage by 75 percent for cosine-normalized vectors, and a two-stage search algorithm that balances speed and accuracy according to application requirements \cite{hnsw,simd,pq}. These optimizations can enable larger datasets on commodity hardware while targeting low-latency query serving in tested configurations. The quantization technique converts float32 representations to int8, while SIMD acceleration improves throughput on compatible hardware.
...
The shard replica placement algorithm considers rack, zone, and region topology and avoids placing all replicas in a single failure domain. This improves fault tolerance in multi-zone deployments and balances load according to node capacity.
...
Vectron combines Raft-based metadata consistency with optimized HNSW indexing, SIMD acceleration, and caching to target both correctness and performance in a single architecture.
...
The Authentication Service handles user authentication through email and password with bcrypt hashing \cite{bcrypt}, JWT token issuance with configurable expiration \cite{jwt}, API key management for programmatic access, and access control enforcement. It stores user credentials in etcd \cite{etcd} with proper hashing and never stores passwords in plaintext. The service provides both gRPC and HTTP interfaces for flexibility.
...
\hline
\centering\textbf{Vectron Architecture Overview} \tabularnewline
\hline
\textbf{Client Layer:} External applications and SDKs issue authenticated REST/gRPC requests for collection management, vector upsert, and similarity search. \tabularnewline
\hline
\textbf{Gateway Layer (Entry Point):} API Gateway validates JWT/API keys, resolves shard routing, performs fanout, applies cache policy, and merges global top-k results. \tabularnewline
\hline
\multicolumn{1}{|c|}{\textbf{Control Plane and Data Plane (Parallel Responsibilities)}} \tabularnewline
\hline
\begin{minipage}[t]{0.47\textwidth}
\textbf{Control Plane}\\
Placement Driver with Raft metadata consensus\\
Worker registry and heartbeat-based health tracking\\
Shard placement, rebalancing, and failover orchestration\\
Authentication service and optional reranker coordination
\end{minipage}
\hfill
\begin{minipage}[t]{0.47\textwidth}
\textbf{Data Plane}\\
Worker shard replicas as per-shard Raft groups\\
Pebble persistence and HNSW index maintenance\\
Vector ingestion, local ANN execution, and partial top-k generation\\
Optional search-only replicas fed by WAL streaming
\end{minipage}
\tabularnewline
\hline
\textbf{Consistency Boundary:} Writes are acknowledged after shard-level Raft quorum commit; read mode is configurable (linearizable or stale/non-linearizable) by policy. \tabularnewline
\hline
\end{tabular}
\caption{Layered Vectron architecture with explicit request path, plane responsibilities, and consistency boundary}
\label{fig:architecture}
\end{figure*}

Figure~\ref{fig:architecture} should be read top-down: client traffic always enters through the API Gateway, while control-plane services coordinate placement and health decisions in parallel with data-plane query/storage execution. This separation keeps metadata governance strongly consistent without placing control operations directly on the latency-critical search path.
...
The request router hashes vector IDs using FNV-1a (64-bit) \cite{fnv} and selects a shard whose precomputed key range contains the hash value. The Placement Driver assigns these hash ranges when creating collections, defaulting to 16 shards unless overridden at creation time. This hash-based distribution is related to classic consistent-hashing design goals for balancing and hotspot reduction in distributed systems \cite{consistenthashing}. For search, the Gateway can either fan out to all shards (default) to ensure full recall, or route to a single shard using a route key derived from the query when fanout is disabled for reduced latency or debugging. The Gateway uses cached routing information from the Placement Driver to minimize coordination overhead.
...
\hline
\multicolumn{3}{|c|}{\textbf{Operational Data Flow}} \\
\hline
\textbf{Step} & \textbf{Write Path (Upsert/Ingestion)} & \textbf{Read Path (Search)} \\
\hline
1 & Client sends request to Gateway; Gateway authenticates and resolves collection route. & Client sends query to Gateway; Gateway authenticates and parses top-k and read policy. \\
2 & Gateway partitions vectors by shard and forwards batched upserts to shard leaders. & Gateway checks cache; on miss it resolves shard topology and fans out query RPCs. \\
3 & Shard leader proposes the batch to Raft and replicates entries to follower replicas. & Workers execute HNSW search on local shard state (leader or eligible follower for stale-read mode). \\
4 & Majority quorum commit establishes durable write completion. & Workers return local top-k candidates and scores to the Gateway. \\
5 & State machine persists vectors in Pebble; background workers update HNSW asynchronously. & Gateway performs global merge, optional reranking, and response assembly. \\
6 & Gateway returns success after commit (search visibility may trail during async indexing). & Gateway writes cache entry (TTL policy) and returns final ranked top-k response. \\
\hline
\multicolumn{3}{|p{0.94\textwidth}|}{\textbf{Key boundary:} durability is guaranteed at Step 4 on the write path (Raft commit), while search visibility is finalized after the asynchronous index-update stage at Step 5.} \\
\hline
\end{tabular}
\caption{Step-aligned write/read operational flow with explicit durability and visibility boundaries}
\label{fig:dataflow}
\end{figure*}

Figure~\ref{fig:dataflow} makes the write and read lifecycles directly comparable by step index. The write path is commit-gated for durability, whereas the read path is latency-optimized through caching and parallel shard fanout, with optional staleness depending on configured read mode and indexing lag.
...
Key dependencies include Dragonboat version 3 providing the Raft consensus implementation and IOnDiskStateMachine interface for persistent state machines \cite{dragonboat}, Pebble from Cockroach Labs as the LSM-tree storage engine \cite{pebble}, gRPC for inter-service communication with Protocol Buffers serialization \cite{grpc,protobuf}, grpc-gateway v2 for HTTP to JSON transcoding enabling REST API compatibility, etcd for authentication service credential storage providing distributed consistency \cite{etcd}, and go-redis for the optional distributed cache layer \cite{redis}. The worker binary is built with CGO enabled and can include an \texttt{avx512} build tag to enable AVX-512 SIMD paths when supported.
...
The experimental evaluation was conducted on a single host featuring an Intel Core i3 (7th Gen U-series) CPU, 8 GB RAM, and Fedora 43. To evaluate the system's distributed architecture, all microservices were co-located on the host, including three Placement Driver nodes, two workers, an API Gateway, an Authentication service, and a Reranker. Metadata and authentication states were managed via \textit{etcd} 3.6.7 and \textit{Valkey} (Redis-compatible), both deployed within Podman containers.

The software stack utilizes Go 1.24.0, Dragonboat 3.3.8 for Raft consensus, and Pebble (v0.0.0-20210331) for persistent storage.

\subsection{Workloads and Scenarios}
The system was subjected to four experimental scenarios designed to assess performance under varying data volumes, dimensional complexities, and load conditions:

\begin{itemize}
    \item \textbf{Scenario 1 (Vector Search Scalability):} Measures insert throughput, search throughput, and latency as the dataset scales from $1,000$ to $10,000$ vectors. Benchmarks were conducted for baseline dimensions of $128D$, $384D$, and $768D$. Each search phase consists of $1,000$ queries. Table II includes $1042D$ and $1568D$ rows, derived via progression-aligned extrapolation from the empirical series.
    \item \textbf{Scenario 2 (Dimension Impact Analysis):} Analyzes the correlation between vector dimensionality and system overhead. Measurements for insert throughput and search latency were taken across dimensions from $64D$ to $1024D$ for $k \in \{1, 5, 10, 50, 100\}$. Table III provides additional projections for $1042D$ and $1568D$ based on high-dimension regime extrapolation.
    \item \textbf{Scenario 3 (Concurrent Workload Analysis):} Evaluates system resilience and throughput saturation by incrementally increasing the number of concurrent clients from $1$ to $100$.
    \item \textbf{Scenario 4 (Distributed System Scalability):} Assesses horizontal scaling efficiency by reporting shard distribution and load balancing across the two-worker configuration.
\end{itemize}

\subsection{Metrics and Measurement}
Performance is quantified using three primary metrics:
\begin{enumerate}
    \item \textbf{Throughput:} Measured in vectors per second (insertion) and queries per second (search).
    \item \textbf{Latency:} Reported at $P_{50}$, $P_{95}$, and $P_{99}$ percentiles to capture the tail-latency distribution.
    \item \textbf{Memory Usage:} Memory deltas are tracked via the benchmark harness to monitor resource efficiency.
\end{enumerate}
...
\midrule
128 & 1000  & 170 & 873 & 1.131 & 1.032 & 1.671 & 2.874 \\
128 & 2500  & 573 & 785 & 1.258 & 1.134 & 2.304 & 2.894 \\
128 & 5000  & 1112 & 355 & 2.799 & 2.869 & 4.070 & 5.073 \\
128 & 10000 & 1122 & 236 & 4.227 & 4.018 & 5.716 & 6.838 \\
384 & 1000  & 229 & 720 & 1.359 & 1.247 & 2.350 & 2.764 \\
384 & 2500  & 500 & 580 & 1.696 & 1.345 & 3.083 & 4.392 \\
384 & 5000  & 781 & 268 & 3.702 & 3.479 & 5.320 & 6.128 \\
384 & 10000 & 1266 & 184 & 5.412 & 5.059 & 7.491 & 9.478 \\
768 & 1000  & 159 & 661 & 1.463 & 1.316 & 2.452 & 3.497 \\
768 & 2500  & 464 & 497 & 1.970 & 1.556 & 3.429 & 5.431 \\
768 & 5000  & 878 & 241 & 4.102 & 3.847 & 5.976 & 7.593 \\
768 & 10000 & 737 & 150 & 6.636 & 6.020 & 10.637 & 14.459 \\
1042 & 1000  & 126 & 608 & 1.595 & 1.434 & 2.673 & 3.812 \\
1042 & 2500  & 367 & 457 & 2.147 & 1.696 & 3.738 & 5.920 \\
1042 & 5000  & 694 & 222 & 4.471 & 4.193 & 6.514 & 8.277 \\
1042 & 10000 & 582 & 138 & 7.233 & 6.562 & 11.594 & 15.760 \\
1568 & 1000  & 62 & 516 & 1.873 & 1.684 & 3.139 & 4.476 \\
1568 & 2500  & 180 & 388 & 2.522 & 1.992 & 4.389 & 6.951 \\
1568 & 5000  & 341 & 188 & 5.251 & 4.924 & 7.648 & 9.718 \\
1568 & 10000 & 286 & 117 & 8.494 & 7.707 & 13.612 & 18.503 \\
\bottomrule
\end{tabular}
\end{table*}
...
\midrule
64   & 3164 & 2.912/3.678 & 3.040/3.974 & 3.312/4.739 & 5.048/7.272 \\
128  & 3186 & 3.532/6.253 & 6.272/10.049 & 6.428/11.287 & 6.119/9.531 \\
256  & 2700 & 3.954/5.312 & 4.146/5.692 & 4.303/7.515 & 6.387/9.575 \\
384  & 2328 & 4.772/7.959 & 5.913/9.968 & 4.651/7.892 & 7.752/15.588 \\
512  & 2230 & 4.576/7.830 & 4.695/5.862 & 5.075/6.939 & 7.724/10.706 \\
768  & 1898 & 4.616/5.567 & 4.776/5.719 & 5.024/6.477 & 7.458/9.042 \\
1024 & 1526 & 4.869/7.561 & 5.008/6.617 & 5.311/7.312 & 7.739/9.657 \\
1042 & 1500 & 4.887/7.701 & 5.024/6.680 & 5.331/7.371 & 7.759/9.700 \\
1568 & 736  & 5.406/11.799 & 5.501/8.526 & 5.921/9.087 & 8.336/10.964 \\
\bottomrule
\end{tabular}
\end{table*}
...
\midrule
1   & 1166 & 40  & 37.195 & 35.315 & 54.725 / 77.006 \\
5   & 3603 & 120 & 60.376 & 57.277 & 89.212 / 116.872 \\
10  & 6444 & 216 & 67.353 & 63.391 & 110.513 / 153.346 \\
20  & 13537& 451 & 64.247 & 63.398 & 95.040 / 110.856 \\
50  & 16023& 534 & 135.177& 137.603& 181.520 / 208.494 \\
100 & 16236& 540 & 265.161& 269.544& 351.002 / 400.598 \\
\bottomrule
\end{tabular}
\end{table*}

\subsection{Scenario 4: Distributed System Scalability}

\begin{table}[htbp]
\centering
\caption{Scenario 4 Results (Shard Distribution)}
\label{tab:scenario4}
\begin{tabular}{@{}cccc@{}}
\toprule
\textbf{Vectors} & \textbf{Shards} & \textbf{Workers} & \textbf{Load Variance (CV)} \\
\midrule
1000  & 16 & 2 (16 shards each) & 0.00 \\
5000  & 16 & 2 (16 shards each) & 0.00 \\
10000 & 16 & 2 (16 shards each) & 0.00 \\
50000 & 16 & 2 (16 shards each) & 0.00 \\
\bottomrule
\end{tabular}
\end{table}
...
In the reported single-host benchmark configuration, search latency remains low at smaller dataset sizes and increases as dataset size and concurrency rise. Tail latency growth is more pronounced under heavier concurrent load. These results are consistent with the expected behavior of fanout-based distributed ANN retrieval on constrained hardware.

Raft-based metadata management reduces the risk of configuration drift and split-brain scenarios that can occur in eventually consistent systems \cite{raft,cap}. This stronger consistency model is important for financial, healthcare, and compliance-sensitive applications where metadata correctness is critical.

Automatic rebalancing, self-healing behavior, and metrics improve operational robustness and reduce manual intervention during overload conditions.

Quantization and graph pruning can improve memory efficiency and make larger datasets feasible on commodity hardware. The resulting infrastructure cost reduction is deployment-dependent and should be validated per workload.
...
LSM-tree storage and Raft replication increase write amplification compared to direct writes \cite{lsm,raft}. SSD endurance must be considered for high-ingestion workloads, though this is mitigated by wear leveling and periodic compaction tuning.

While quantization helps significantly, HNSW still requires substantial RAM for the graph structure \cite{hnsw,pq}. Exabyte-scale datasets would require hundreds of nodes, increasing operational complexity and cost.
...
Strong consistency for metadata introduces additional coordination latency for collection creation and schema changes. This overhead is acceptable for most use cases but may impact workloads requiring very frequent schema modifications.

\subsection{Observed Trade-offs}

Int8 quantization provides substantial memory reduction but can introduce recall degradation for fine-grained similarity tasks; this trade-off is consistent with the broader ANN quantization literature \cite{pq,gpuann}. Applications requiring exact ordering such as duplicate detection should use full-precision storage despite the higher memory cost.
...
Under intensive update workloads (above roughly 1000 updates per second per shard), gradual HNSW graph quality degradation can occur without periodic rebuilds. Background rebuild processes mitigate this effect but consume CPU cycles that could otherwise serve queries.

Without active request coalescing, cache expiration can cause thundering herd effects when many clients simultaneously request expired entries. The TinyLFU policy and caching layers mitigate this \cite{tinyLFU}, but perfectly synchronized clients can still trigger bursty fanout until coalescing is fully integrated.
...
GPU acceleration through CUDA or cuANN integration could provide major speedups on large batch queries \cite{gpuann}. The modular HNSW implementation can be extended with GPU-accelerated distance kernels while maintaining the existing CPU fallback paths.
...
Disaggregated storage, separating compute for HNSW traversal from vector storage, would enable independent scaling. Remote storage technologies such as NVMe over Fabrics or RDMA could support shared stateless workers over centralized storage.
...
This paper presented Vectron, a distributed vector database that combines Raft-based consensus, optimized HNSW indexing with SIMD acceleration, and multi-tier caching to balance consistency and performance for large-scale vector search workloads. The system architecture makes explicit trade-offs between consistency, availability, and performance for production-oriented deployments.
...
Overall, the current implementation indicates that strong metadata consistency and practical query performance can be combined in a single architecture when system-level trade-offs are made explicit. The open-source implementation and documentation provide a baseline for further validation, tuning, and extension in larger multi-node deployments.
...
\bibitem{hnsw} Y. A. Malkov and D. A. Yashunin, ``Efficient and robust approximate nearest neighbor search using hierarchical navigable small world graphs,'' \emph{IEEE Transactions on Pattern Analysis and Machine Intelligence}, vol. 42, no. 4, pp. 824--836, Apr. 2020.

\bibitem{raft} D. Ongaro and J. Ousterhout, ``In search of an understandable consensus algorithm,'' in \emph{Proc. USENIX Annual Technical Conf. (ATC)}, 2014, pp. 305--319.

\bibitem{pq} H. J\'egou, M. Douze, and C. Schmid, ``Product quantization for nearest neighbor search,'' \emph{IEEE Transactions on Pattern Analysis and Machine Intelligence}, vol. 33, no. 1, pp. 117--128, Jan. 2011.

\bibitem{lsh} A. Gionis, P. Indyk, and R. Motwani, ``Similarity search in high dimensions via hashing,'' in \emph{Proc. 25th Int. Conf. Very Large Data Bases (VLDB)}, 1999, pp. 518--529.

\bibitem{rag} P. Lewis \emph{et al.}, ``Retrieval-augmented generation for knowledge-intensive NLP tasks,'' in \emph{Advances in Neural Information Processing Systems (NeurIPS)}, vol. 33, 2020, pp. 9459--9474.

\bibitem{gpuann} J. Johnson, M. Douze, and H. J\'egou, ``Billion-scale similarity search with GPUs,'' \emph{IEEE Transactions on Big Data}, vol. 7, no. 3, pp. 535--547, Jul.--Sep. 2021.

\bibitem{milvus} Milvus Authors, ``Milvus documentation,'' [Online]. Available: https://milvus.io/docs.

\bibitem{weaviate} Weaviate Authors, ``Weaviate documentation,'' [Online]. Available: https://weaviate.io/developers/weaviate.

\bibitem{tinyLFU} G. Einziger, R. Friedman, and B. Manes, ``TinyLFU: A highly efficient cache admission policy,'' \emph{ACM Transactions on Storage}, vol. 13, no. 4, pp. 1--31, 2017.

\bibitem{dragonboat} L. Ni, ``Dragonboat: A high performance multi-group Raft library in Go,'' [Online]. Available: https://github.com/lni/dragonboat.

\bibitem{faiss} J. Johnson, M. Douze, and H. J\'egou, ``Faiss: A library for efficient similarity search and clustering of dense vectors,'' [Online]. Available: https://github.com/facebookresearch/faiss.

\bibitem{annoy} E. Bernhardsson, ``Annoy: Approximate nearest neighbors in C++/Python,'' [Online]. Available: https://github.com/spotify/annoy.

\bibitem{pebble} Cockroach Labs, ``Pebble: A Go key-value store,'' [Online]. Available: https://github.com/cockroachdb/pebble.

\bibitem{grpc} gRPC Authors, ``gRPC: A high performance, open source universal RPC framework,'' [Online]. Available: https://grpc.io.

\bibitem{protobuf} Google, ``Protocol Buffers,'' [Online]. Available: https://protobuf.dev.

\bibitem{jwt} M. Jones, J. Bradley, and N. Sakimura, ``JSON Web Token (JWT),'' RFC 7519, May 2015.

\bibitem{bcrypt} N. Provos and D. Mazi\`eres, ``A Future-Adaptable Password Scheme,'' in \emph{Proc. USENIX Annual Technical Conf.}, 1999.

\bibitem{etcd} etcd Authors, ``etcd: A distributed, reliable key-value store for the most critical data of a distributed system,'' [Online]. Available: https://etcd.io.

\bibitem{lsm} P. O'Neil, E. Cheng, D. Gawlick, and E. O'Neil, ``The log-structured merge-tree (LSM-tree),'' \emph{Acta Informatica}, vol. 33, no. 4, pp. 351--385, 1996.

\bibitem{cap} E. A. Brewer, ``CAP twelve years later: How the `rules' have changed,'' \emph{Computer}, vol. 45, no. 2, pp. 23--29, Feb. 2012.

\bibitem{simd} Intel, ``Intel\textsuperscript{\textregistered} Advanced Vector Extensions 2 (Intel\textsuperscript{\textregistered} AVX2),'' [Online]. Available: https://www.intel.com/content/www/us/en/developer/articles/technical/intel-advanced-vector-extensions.html.

\bibitem{consistenthashing} D. R. Karger, E. Lehman, T. Leighton, R. Panigrahy, M. Levine, and D. Lewin, ``Consistent hashing and random trees: Distributed caching protocols for relieving hot spots on the World Wide Web,'' in \emph{Proc. 29th Annu. ACM Symp. Theory of Computing (STOC)}, 1997, pp. 654--663.

\bibitem{fnv} G. Fowler, L. C. Noll, and K. Vo, ``The FNV non-cryptographic hash algorithm,'' [Online]. Available: http://www.isthe.com/chongo/tech/comp/fnv/.

\bibitem{redis} Redis Ltd., ``Redis documentation,'' [Online]. Available: https://redis.io/docs.

\bibitem{qdrant} Qdrant Authors, ``Qdrant documentation,'' [Online]. Available: https://qdrant.tech/documentation/.

\bibitem{pgvector} pgvector Authors, ``pgvector: Open-source vector similarity search for PostgreSQL,'' [Online]. Available: https://github.com/pgvector/pgvector.
