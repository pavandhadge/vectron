...
export const ApiKeyManager: React.FC = () => {
  ...
  const fetchKeys = useCallback(async () => {
    ...
    try {
      ...
      setError(
        "Failed to fetch API keys. Your session may have expired or you might not have access.",
      );
  ...
  const handleDeleteKey = async (keyPrefix: string) => {
    ...
    try {
      // Using global axios
      await apiClient.delete(`/v1/keys/${keyPrefix}`);
      fetchKeys(); // Refresh the list
    } catch (err: any) {
      setError(
        err.response?.data?.message || `Failed to delete key ${keyPrefix}.`,
      );
    ...
    <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
      ...
      {!loading && keys.length === 0 && (
        <p className="text-gray-600 dark:text-gray-400">
          You have no API keys. Create one above to get started.
        </p>
      )}
      {keys.length > 0 && (
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600 dark:text-gray-300">
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className="text-blue-600 hover:text-blue-900 dark:text-blue-400 dark:hover:text-blue-300 text-xs transition"
                      >
                        Copy
                      </button>
                    </div>
