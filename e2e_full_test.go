...
func TestE2E_FullSystem_WithAuth(t *testing.T) {
	...
	//    b. Create an API key for that user.
	// 4. Using the Go client library with the real API key to interact with the apigateway.
	...
	require.NoError(t, err)
		...
		// --- 4. Get API Key from Auth Service ---
		...
		t.Log("Successfully logged in and obtained JWT")
		...
		apiKey, err := authClient.CreateAPIKey(ctx, apiKeyName)

		require.NoError(t, err)

		require.NotEmpty(t, apiKey)

		t.Logf("Successfully created API key: %s...", apiKey[:13])

	

		// --- 5. Run Test Logic using Go Client ---

		client, err := vectron.NewClient(gatewayAddr, apiKey)
